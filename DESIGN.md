...
- 检索与向量化选型：本项目暂不启用 Chroma/RAG。当前活动量为 41 条，关键词检索足以支持全文、类别和时间范围查询，且结果可复现、可离线验收；已有 9 个确定性工具满足“至少 3 个工具”的底线。若后续活动量变大或需要语义查询，再引入 Chroma 作为第二路召回，并保留规则检索保证结果可解释。
...
- 前端活动区提供「日历 / 列表」双视图：默认月视图日历，点击日期展示当天活动，无具体时间的活动归入「待定时间」区；列表视图保留原表格便于批量操作。
