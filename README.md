...
校园活动与通知聚合 Agent。系统汇集校园讲座、竞赛、社团招新、教务通知、文体活动、志愿服务等通知，支持全文检索、类别筛选、时间范围筛选，并把 `第 X 周周三`、`11.15`、`周三下午`、`本月`、`下周` 等自然语言时间统一成具体日期。
...
- 26 条手工整理活动数据、15 条南林校网抓取活动，共 41 条，覆盖 6 个类别和多种时间格式；另有本地示例源 `data/feed_candidates.json`，点击“抓取更新”可演示新增、去重与人工确认（新增 2 条、跳过重复 1 条、待确认 1 条）。
...
- Web 界面：标准库 HTTP 服务，无第三方前端依赖；活动区支持「日历 / 列表」双视图，默认月视图日历，点击日期在下方列出当天活动，无具体时间的活动单独归入「待定时间」区。
...
运行 `python run.py` 后，浏览器打开 `http://127.0.0.1:8000`。页面左侧活动区默认以月视图日历展示，支持「上月/下月/今天」切换、点击日期查看当天活动，并可切回列表视图；右侧可进行 Deep Agent（多智能体）对话、查看提醒和待审核记录。


## 测试

后端共 64 个单元测试全部通过，其中时间解析专项使用 13 个固定表达验证、准确率 100%，用例见 `tests/test_cases.md`；前端「日历 / 列表」双视图另附手动验证用例 TC-19 / TC-20。
