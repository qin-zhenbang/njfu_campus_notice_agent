...
`用户输入 -> LangChain Agent -> 工具调用 -> ChatOpenAI -> 输出`

活动检索、时间解析、提醒、兴趣匹配、抓取去重和人工审核由确定性工具实现。右侧 Agent 使用 LangChain `create_agent` 调用这些工具，每次请求最多调用 10 次；未配置或调用失败时自动降级为本地回复。
...
- 25 条手工整理活动数据、2 条本地抓取演示数据、15 条南林校网抓取活动，共 42 条，覆盖 6 个类别和多种时间格式。
- 自然语言时间解析：`第 X 周周 X`、`11.15`、`X月X日`、`周三下午`、`本周/下周/本月/下个月/明天/后天` 等。
- 全文检索、类别筛选、时间范围筛选，统一输出活动名称、标准化时间、地点、类别、来源。
- LangChain Agent：右侧栏可调用活动检索、提醒、兴趣、抓取、待审核、统计，以及手动新增/编辑活动工具。
...
- 到点自动提醒：提醒写入 JSON 长期记忆，后台按配置间隔检查，到期后页面自动显示并可完成。
...
| `POST /api/events/delete` | 删除活动（级联取消提醒、移除通知） | `{ "id": "EVT-041" }` |
...
- 删除活动时，引用该活动的未完成提醒会标记为 `cancelled`，对应兴趣推送会被移除。
...
LangChain Agent 默认读取 `.env` 中的 LLM 配置，可复制 `.env.example` 为 `.env` 并填写：
...
没有 `.env` 或接口不可用时，聊天自动降级为本地回复，活动筛选等功能不受影响。
...
| 成员 | 负责模块 | 说明 |
|---|---|---|
| 成员 A | 数据整理、时间解析 | 活动数据字段设计与清洗 |
| 成员 B | Agent、检索、提醒 | 核心工具与对话流程 |
| 成员 C | 抓取、人工审核、文档 | 数据源接入与测试 |
...
运行 `python run.py` 后，浏览器打开 `http://127.0.0.1:8000`。页面左侧可筛选活动，右侧可进行 LangChain Agent 对话、查看提醒和待审核记录。
